...
class PoseNet(nn.Module):
    ...
    def conv_block(self, channel_in, channel_out, kernel_size, stride, padding, bias):
      ...
      # 10/11/22 Changed back to ReLU() from LeakyReLU() since NaNs seem to be coming from original warping function
    ...
    def forward(self, xp):
        ...
        #a = a_decoder(xp)
        #a = self.a_out(xp)
        a = torch.mean(self.a_out(xp),3).view(par.batch_size,1)
        #print(a.shape)
        ...
        #b = b_decoder(xp)
        #b = self.b_out(xp)
        b = torch.mean(self.b_out(xp),3).view(par.batch_size,1)
        #print(b.shape)
        ...
        #xp = self.pose_out(xp)
        xp = torch.mean(self.pose_out(xp),3).view(par.batch_size,6) # 1e-2
        #print(xp)
        #print(xp.shape)
...
'''example_sequence = torch.randint(0,100,(batch_size,)).tolist()
training_data = D3VODataset(img_dir=img_dir, img_dirs=train_dirs, 
                            transform=transform, batch_size=batch_size, 
                            network_type='pose')
poses = training_data.__getitem__(0,1,example_sequence)

pose_test,a,b = posenet_model(poses)

print("PoseNet Shape: ")
print(pose_test.shape)
